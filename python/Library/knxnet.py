...
__copyright__ = "HES-SO 2017, Project EMG4B"
__credits__ = ["Adrien Lescourt", "Bouchedakh Mohamed Nizar ", "Adriano Silva"]
__version__ = "1.1.0"
...
class TunnellingRequest(KnxnetFrame):
    ...
    def create_from_frame(cls, frame):
        ...
        if data_size > 8:
           raise KnxnetException('Invalid frame: Unsupported datapoint type (data size > 8')
        ...
        if data_size == 1:  # boolean
            ...
            data = ''
            while(index<=18):
                if int(raw_body[index])<0xa:
                    toConcat = '0'+ str(raw_body[index])
                    index = index+1
                else:
                    toConcat = hex(raw_body[index])
                    index =index+1
                data= data + toConcat
            data = re.sub('0x', '', data)
